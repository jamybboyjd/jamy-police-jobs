...
RegisterNetEvent('esx_sheriff:confiscatePlayerItem')
AddEventHandler('esx_sheriff:confiscatePlayerItem', function(target, itemType, itemName, amount)
	...
	if sourceXPlayer.job.name ~= 'sheriff' then
		print(('esx_sheriff: %s attempted to confiscate!'):format(sourceXPlayer.identifier))
		return
	end
...
AddEventHandler('esx_sheriff:handcuff', function(target)
	...
	if xPlayer.job.name == 'sheriff' then
		...
		print(('esx_sheriff: %s attempted to handcuff a player (not cop)!'):format(xPlayer.identifier))
	end
end)
...
AddEventHandler('esx_sheriff:drag', function(target)
	...
	if xPlayer.job.name == 'sheriff' then
		...
		print(('esx_sheriff: %s attempted to drag (not cop)!'):format(xPlayer.identifier))
	end
end)
...
AddEventHandler('esx_sheriff:putInVehicle', function(target)
	...
	if xPlayer.job.name == 'sheriff' then
		...
		print(('esx_sheriff: %s attempted to put in vehicle (not cop)!'):format(xPlayer.identifier))
	end
end)
...
AddEventHandler('esx_sheriff:OutVehicle', function(target)
	...
	if xPlayer.job.name == 'sheriff' then
		...
		print(('esx_sheriff: %s attempted to drag out from vehicle (not cop)!'):format(xPlayer.identifier))
	end
end)

RegisterNetEvent('esx_sheriff:getStockItem')
...
RegisterNetEvent('esx_sheriff:putStockItems')
...
ESX.RegisterServerCallback('esx_sheriff:getOtherPlayerData', function(source, cb, target, notify)
...
ESX.RegisterServerCallback('esx_sheriff:addArmoryWeapon', function(source, cb, weaponName, removeWeapon)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('esx_sheriff:buyWeapon', function(source, cb, weaponName, type, componentNum)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if not selectedWeapon then
		print(('esx_sheriff: %s attempted to buy an invalid weapon.'):format(xPlayer.identifier))
		cb(false)
	else
		-- Weapon
		if type == 1 then
		...
		elseif type == 2 then
			...
			if component then
				...
				print(('esx_sheriff: %s attempted to buy an invalid weapon component.'):format(xPlayer.identifier))
				cb(false)
			end
		end
	end
...
ESX.RegisterServerCallback('esx_sheriff:buyJobVehicle', function(source, cb, vehicleProps, type)
	local xPlayer = ESX.GetPlayerFromId(source)
	local price = getPriceFromHash(vehicleProps.model, xPlayer.job.grade_name, type)

	-- vehicle model not found
	if price == 0 then
		print(('esx_sheriff: %s attempted to exploit the shop! (invalid vehicle model)'):format(xPlayer.identifier))
		cb(false)
	else
		if xPlayer.getMoney() >= price then
			xPlayer.removeMoney(price)
...
ESX.RegisterServerCallback('esx_sheriff:storeNearbyVehicle', function(source, cb, nearbyVehicles)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if not foundPlate then
		...
		MySQL.Async.execute('UPDATE owned_vehicles SET `stored` = true WHERE owner = @owner AND plate = @plate AND job = @job', {
			...
			if rowsChanged == 0 then
				print(('esx_sheriff: %s has exploited the garage!'):format(xPlayer.identifier))
				cb(false)
			else
				cb(true, foundNum)
			end
...
AddEventHandler('playerDropped', function()
	...
	if playerId then
		...
		if xPlayer and xPlayer.job.name == 'sheriff' then
			Citizen.Wait(5000)
...
RegisterNetEvent('esx_sheriff:spawned')
...
RegisterNetEvent('esx_sheriff:forceBlip')
